...
def bag(items_info: List[int], capacity: int) -> int:
    """
    Static bag, calculate the max weight combination
    :param items_info:
    :param capacity:
    :return:
    """
    ...
    for i in range(1, n):
        for cur_weight in range(capacity+1):
            if memo[i-1][cur_weight] != -1:
                memo[i][cur_weight] = memo[i-1][cur_weight] #not chosen
                if cur_weight + items_info[i] <= capacity:  #chosen
...
def bag_with_max_value(items_info: List[Tuple[int, int]], capacity: int) -> int:
    """
    Static bag, calculate the max value combination
    :param items_info:
    :param capacity:
    :return:
    """
